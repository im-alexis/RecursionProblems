...
int minRec2(int x[], int n) {
    if(n == 1){ // if the length is 1 then x[0] is auto the smallest
        return x[0];
    }
    else{
        int smolLEFT = minRec2(x, n / 2); // this gets you the values of the left side of the array
        int smolRIGHT = minRec2((x + n/2), n - (n/2)); // move the pointer of the array n/2 to get the other half of the array change
        // change the length of the right side by subtracting the n/2 r from n after getting rid of the elememts on the left. Cant use modulo I get zero when even
        if (smolLEFT > smolRIGHT) { // right number is less than the left number
            return smolRIGHT;
        }
        else {
            return smolLEFT;// left number is less than the right number
        }
    }
 ...
 * Since you can't calculate a square root exactly, for this problem
 * you're required to calculate the answer to 15 decimal digits of
 * accuracy. 0.000000000000001 -> increments by 1*10^-15
 */
double sqrtRec(double x, double low_guess, double high_guess) {

    if (low_guess <= high_guess) {
        double num = (low_guess + high_guess)/2;
        if (num * num <= x && ((num + 0.000000000000001) * (num + 0.000000000000001) > x)) {
            return num;
        }
        if (num * num < x) {
            return sqrtRec(x, num + 0.000000000000001, high_guess);
        }
        if (num * num > x) {
            return sqrtRec(x, low_guess, num - 0.000000000000001);
        }
    }
}
...
int strCompare(char* str1, char* str2) {
    //int recStr = -1;
        ...
        if (*str1 == '\0' && *str2 == '\0') { // reached the end of the string so its equal
            return  0;
        }
       else if (*str1 > *str2) {
            return  1;
    ...
    return strCompare(str1+1 , str2 +1); //moves on from the letters
}
...
int strCompare2(char* str1, char* str2) {
    //int recStr;
    if (whatLetter(*str1) == -1 && *str1 != '\0'  ){ // if str1 is not a letter and is not the end move to the next character
        return strCompare2(str1 + 1, str2);
    }
    if (whatLetter(*str2) == -1 && *str2 != '\0' ) { // if str2 is not a letter and is not the end move to the next character
    ...
    if(*str1 == '\0' && *str2 == '\0'){
        ...
        return  -1; // str2 is greater that str1
    }
    return strCompare2((str1 + 1), (str2 + 1)); // keep checking
}
...
/* Will return a 1 if it is possible to travel in the direction moved into the function
 * you wont be able to if there is a wall(1) good crum (2) bad crum (4)
 *  N -> [row] [col+1]
 *  S -> [row] [col-1]
 *  W -> [row - 1] [col]
 *  E -> [row + 1] [col]
 * */
bool isPossibleNESW(int row, int col){
...
//Checks if a spot is a good crum
bool isGoodCrum(int row , int col){
    if(maze[row][col] == 2){
        return true;
    }
    return false;
}
/*Will go back one the direction chosen -----> DIDN"T USE better when its solution
 * Put down a badBreadcrum to show that way is wasted
 *
int backtrack(int row, int col){
    if (!isPossibleNESW(row , col + 1) && !isPossibleNESW(row - 1 , col) && !isPossibleNESW(row + 1 , col) && isGoodCrum(row , col - 1)) { // Backtracking south
        maze[row][col - 1] = 4;
        backtrack(row, col - 1);
    }
    else if (!isPossibleNESW(row , col - 1) && !isPossibleNESW(row - 1 , col) && !isPossibleNESW(row + 1 , col) &&isGoodCrum(row, col + 1)) { // backtracking north
        maze[row][col + 1] = 4;
        backtrack(row, col + 1);
    }
    else if (!isPossibleNESW(row , col + 1) && !isPossibleNESW(row - 1 , col) && !isPossibleNESW(row, col - 1) &&isGoodCrum(row + 1, col)) { // backtracking east
        maze[row + 1][col] = 4;
        backtrack(row + 1, col);
    }
    else if (!isPossibleNESW(row , col + 1) && !isPossibleNESW(row + 1 , col) && !isPossibleNESW(row, col - 1) && isGoodCrum(row - 1, col)) {// backtracking west
        maze[row - 1][col] = 4;
        backtrack(row - 1, col);
    }
    else{
...
/*Keeping bad crums along side good makes life easy so
 * keep them till the end
 * */
void removeBadCrums(){
    int i;
    for(i = 0; i < MATRIX_SIZE ; i++){//row
        int k;
        for(k = 0; k < MATRIX_SIZE ; k++){//col
            if(maze[i][k] == 4){
                maze[i][k] = 0;
            }
        }
...
// BAD CRUM will be a 4
//GOOD CRUM will be a 2
int solveMazeRec(int row, int col) {
    if(row == 0){ // just so it looks nice puts a crum at the gate
       maze[row][col] = 2;
    }
    if(row == MATRIX_SIZE  - 1 && maze[row][col] != 1){ // when it is solved
        maze[row][col] = 2;
        printf("YUUURRR Solution found :) \n");
        removeBadCrums(); // clean up the path of the bad crums
        //return;
    }
    else if(isPossibleNESW(row, col - 1)){ // moving south
        maze[row][col - 1] = 2;
        solveMazeRec(row, col - 1);
    }
    else if(isPossibleNESW(row, col + 1)){ // moving north
        maze[row][col + 1] = 2;
        solveMazeRec(row, col + 1);
    }
    else if(isPossibleNESW(row + 1, col)){ // moving east
        maze[row + 1][col] = 2;
        solveMazeRec(row + 1, col);
    }
    else if(isPossibleNESW(row - 1, col)){ // moving west
        maze[row - 1][col] = 2;
        solveMazeRec(row - 1, col);
    }
    else { // goes into these when you get stuck go back and change good to bad crums
        if (!isPossibleNESW(row , col + 1) && !isPossibleNESW(row - 1 , col)
        && !isPossibleNESW(row + 1 , col) && !isPossibleNESW(row, col -1) && isGoodCrum(row , col - 1)) { // Backtracking south
            maze[row][col] = 4;
            solveMazeRec(row, col - 1);
        }
        else if (!isPossibleNESW(row , col - 1) && !isPossibleNESW(row - 1 , col)
        && !isPossibleNESW(row + 1 , col) && !isPossibleNESW(row, col + 1) && isGoodCrum(row, col + 1)) { // backtracking north
            maze[row][col] = 4;
            solveMazeRec(row, col + 1);
        }
        else if (!isPossibleNESW(row , col + 1) && !isPossibleNESW(row - 1 , col)
        && !isPossibleNESW(row, col - 1) && !isPossibleNESW(row + 1 , col) &&isGoodCrum(row + 1, col)) { // backtracking east
            maze[row][col] = 4;
            solveMazeRec(row + 1, col);
        }
        else if (!isPossibleNESW(row , col + 1) && !isPossibleNESW(row + 1 , col)
        && !isPossibleNESW(row, col - 1) && !isPossibleNESW(row - 1 , col ) && isGoodCrum(row - 1, col)) {// backtracking west
            maze[row][col] = 4;
            solveMazeRec(row - 1, col);
        }
       else{printf("OOOHH FOK!!! NO Solution :(\n");
       }
    }
...
Martian change(int cents) {
    ...
    Martian holder {.pennies = 0, .nicks = 0, .dodeks = 0};
    if(cents == 0){
        return holder;
    }
    if (cents >= 12 && cents % 12 < 3 ){ // if you have a modulo greater that 3 that means you could make another coin instead of using pennies
        holder = change(cents - 12);// 1d + (>3p) will always be more coins than 3n + (whatever p)
        holder.dodeks ++;

    }
    else if (cents >= 5 && cents % 5 <= 4 ){
        holder =  change(cents - 5);
        holder.nicks ++;

    }
   else if (cents >=  1){
        holder = change(cents - 1);
        holder.pennies ++;
...
Martian change(int cents, int nick_val, int dodek_val) {
    ...
    if (cents >= dodek_val && cents % dodek_val <= (dodek_val%nick_val)){ // want less than nicks so you won't complete in nicks
        holder = change(cents - dodek_val,nick_val, dodek_val);
        holder.dodeks ++;
    }
    else if (cents >= nick_val && cents % nick_val <= nick_val -1 ){
        holder =  change(cents - nick_val, nick_val, dodek_val);
        holder.nicks ++;
    }
    else if (cents >=  1){
        holder = change(cents - 1, nick_val, dodek_val );
        holder.pennies ++;
    }
    return holder;
}
